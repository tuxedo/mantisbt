...
class Period {
	...
	*/
	...
	function period_selector( $p_control_name ) {
		...
		$t_ret .= "</div>\n";
		$t_ret .= "<div id=\"dates\">\n";
		$t_ret .= '<label for="start_date">' . _( 'From' ) . '</label><input type="text" id="start_date" name="start_date" size="20" value="' . $t_formatted_start . '" class="datetime" disabled="disabled" />' . "<br />\n";
		$t_ret .= '<label for="end_date">' . _( 'To' ) . '</label><input type="text" id="end_date" name="end_date" size="20" value="' . $t_formatted_end . '" class="datetime" disabled="disabled" />' . "\n";
		$t_ret .= "</div>\n";
		return $t_ret;
	}
