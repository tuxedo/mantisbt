...
function tag_exists( $p_tag_id ) {
	$t_query = 'SELECT id FROM {tag} WHERE id=%d';
...
function tag_is_unique( $p_name ) {
	...
	$result = db_query( $t_query, array( $c_name ) );
...
function tag_delete( $p_tag_id ) {
	...
	$t_query = 'DELETE FROM {tag} WHERE id=%d';
	...
	if ( 0 != $p_bug_id ) {
		...
		if ( db_is_mssql() ) {
			...
			$t_query = "SELECT t.id FROM {tag} t
					...
					WHERE b.bug_id IS NULL OR b.bug_id != %d";
			$t_result = db_query( $t_query, $t_params );

			$t_subquery_results = array();

			while( $t_row = db_fetch_array( $t_result ) ) {
				$t_subquery_results[] = (int)$t_row;
			}
...
function tag_stats_related( $p_tag_id, $p_limit = 5 ) {
	...
	$subquery = "SELECT b.id FROM {bug} AS b
					LEFT JOIN {project_user_list} AS p
						ON p.project_id=b.project_id AND p.user_id=%d
					JOIN {user} AS u
						ON u.id=%d
					JOIN {bug_tag} AS t
						ON t.bug_id=b.id
