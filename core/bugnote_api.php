...
function bugnote_exists( $p_bugnote_id ) {
	...
	$t_result = db_query( $t_query, array( (int)$p_bugnote_id ) );
...
function bugnote_add( $p_bug_id, $p_bugnote_text, $p_time_tracking = '0:00', $p_private = false, $p_type = 0, $p_attr = '', $p_user_id = null, $p_send_email = TRUE, $p_date_submitted = 0, $p_last_modified = 0, $p_skip_bug_update = FALSE ) {
	...
	if( $p_private && access_has_bug_level( config_get( 'set_view_status_threshold' ), $t_bug, $p_user_id ) ) {
	...
	$t_query = "INSERT INTO {bugnote}
				(bug_id, reporter_id, view_state, date_submitted, last_modified, note_type, note_attr, time_tracking, note )
			VALUES (%d, %d, %d, %d,%d, %d, %s, %d, %s )";
	db_query( $t_query, array( $c_bug_id, $p_user_id, $t_view_state, $c_date_submitted, $c_last_modified, $c_type, $p_attr, $c_time_tracking, $t_bugnote_text ) );
...
function bugnote_get_text( $p_bugnote_id ) {
	...
	$t_query = 'SELECT note FROM {bugnote} WHERE id=%d';
...
function bugnote_get_field( $p_bugnote_id, $p_field_name ) {
	...
	$result = db_query( $query, array( $p_bugnote_id ), 1 );
...
function bugnote_set_text( $p_bugnote_id, $p_bugnote_text ) {
	...
	db_query( $t_query, array( $p_bugnote_text, $p_bugnote_id ) );
