...
echo _( 'Updating Issue Information' );
echo '</td><td class="right" colspan="3">';
print_bracket_link( string_get_bug_view_url( $t_bug_id ), _( 'Back To Issue' ) );
echo '</td></tr>';

# Submit Button
if ( $t_top_buttons_enabled ) {
        echo '<tr><td class="center" colspan="6">';
        echo '<input ', helper_get_tab_index(), ' type="submit" class="button" value="', _( 'Update Information' ), '" />';
        echo '</td></tr>';
}
...
if ( $t_show_id || $t_show_project || $t_show_category || $t_show_view_state || $t_show_date_submitted | $t_show_last_updated ) {
	...
	echo '<tr>';
	echo '<td width="15%" class="category">', $t_show_id ? _( 'ID' ) : '', '</td>';
	echo '<td width="20%" class="category">', $t_show_project ? _( 'Project' ) : '', '</td>';
	echo '<td width="15%" class="category">', $t_show_category ? '<label for="category_id">' . _( 'Category' ) . '</label>' : '', '</td>';
	echo '<td width="20%" class="category">', $t_show_view_state ? '<label for="view_state">' . _( 'View Status' ) . '</label>' : '', '</td>';
	echo '<td width="15%" class="category">', $t_show_date_submitted ? _( 'Date Submitted' ) : '', '</td>';
	echo '<td width="15%" class="category">', $t_show_last_updated ? _( 'Last Update' ) : '', '</td>';
	echo '</tr>';
	...
	if ( $t_can_change_view_state ) {
		...
		print_enum_string_option_list( 'view_state', $t_bug->view_state);
		echo '</select>';
...
if ( $t_show_reporter ) {
	...
	if ( $t_show_reporter ) {
		# Reporter
		echo '<th class="category"><label for="reporter_id">' . _( 'Reporter' ) . '</label></th>';
		echo '<td>';
...
if ( $t_show_handler || $t_show_due_date ) {
	...
	# Assigned To
	echo '<th class="category"><label for="handler_id">' . _( 'Assigned To' ) . '</label></th>';
	echo '<td>';
...
if ( $t_show_priority || $t_show_severity || $t_show_reproducibility ) {
	...
	if ( $t_show_priority ) {
		# Priority
		echo '<th class="category"><label for="priority">' . _( 'Priority' ) . '</label></th>';
	...
	if ( $t_show_severity ) {
		# Severity
		echo '<th class="category"><label for="severity">' . _( 'Severity' ) . '</label></th>';
	...
	if ( $t_show_reproducibility ) {
		# Reproducibility
		echo '<th class="category"><label for="reproducibility">' . _( 'Reproducibility' ) . '</label></th>';
...
if ( $t_show_status || $t_show_resolution ) {
	...
	if ( $t_show_resolution ) {
		# Resolution
		echo '<th class="category"><label for="resolution">' . _( 'Resolution' ) . '</label></th>';
...
if ( $t_show_projection || $t_show_eta ) {
	...
	if ( $t_show_projection ) {
		# Projection
		echo '<th class="category"><label for="projection">' . _( 'Projection' ) . '</label></th>';
	...
	# ETA
	if ( $t_show_eta ) {
		echo '<th class="category"><label for="eta">' . _( 'ETA' ) . '</label></th>';
		echo '<td><select ' . helper_get_tab_index() . ' id="eta" name="eta">';
		print_enum_string_option_list( 'eta', $t_bug->eta );
		echo '</select></td>';
	} else {
		$t_spacer += 2;
	}
...
if ( $t_show_platform || $t_show_os || $t_show_os_version ) {
	...
	if ( $t_show_platform ) {
		# Platform
		echo '<th class="category"><label for="platform">' . _( 'platform' ) . '</label></th>';
		echo '<td>';
	...
	if ( $t_show_os ) {
		# Operating System
		echo '<th class="category"><label for="os">' . _( 'OS' ) . '</label></th>';
		echo '<td>';
	...
	if ( $t_show_os_version ) {
		# OS Version
		echo '<th class="category"><label for="os_build">' . _( 'OS Version' ) . '</label></th>';
		echo '<td>';
...
if ( $t_show_product_version || $t_show_product_build ) {
	...
	if ( $t_show_product_version ) {
		echo '<th class="category"><label for="version">' . _( 'Product Version' ) . '</label></th>';
	...
	if ( $t_show_product_build ) {
		echo '<th class="category"><label for="build">' . _( 'Product Build' ) . '</label></th>';
		echo '<td>';
...
if ( $t_show_target_version || $t_show_fixed_in_version ) {
	...
	if ( $t_show_target_version ) {
		echo '<th class="category"><label for="target_version">' . _( 'Target Version' ) . '</label></th>';
	...
	if ( $t_show_fixed_in_version ) {
		echo '<th class="category"><label for="fixed_in_version">' . _( 'Fixed in Version' ) . '</label></th>';
		echo '<td>';
...
if ( $t_show_summary ) {
	echo '<tr>';
	echo '<th class="category"><label for="summary">' . _( 'Summary' ) . '</label></th>';
...
if ( $t_show_description ) {
	echo '<tr>';
	echo '<th class="category"><label for="description">' . _( 'Description' ) . '</label></th>';
	echo '<td colspan="5">';
	echo '<textarea ', helper_get_tab_index(), ' cols="80" rows="10" id="description" name="description">', $t_description_textarea, '</textarea>';
	echo '</td></tr>';
}
...
if ( $t_show_steps_to_reproduce ) {
	echo '<tr>';
	echo '<th class="category"><label for="steps_to_reproduce">' . _( 'Steps To Reproduce' ) . '</label></th>';
	echo '<td colspan="5">';
	echo '<textarea ', helper_get_tab_index(), ' cols="80" rows="10" id="steps_to_reproduce" name="steps_to_reproduce">', $t_steps_to_reproduce_textarea, '</textarea>';
	echo '</td></tr>';
}
...
if ( $t_show_additional_information ) {
	echo '<tr>';
	echo '<th class="category"><label for="additional_information">' . _( 'Additional Information' ) . '</label></th>';
	echo '<td colspan="5">';
	echo '<textarea ', helper_get_tab_index(), ' cols="80" rows="10" id="additional_information" name="additional_information">', $t_additional_information_textarea, '</textarea>';
	echo '</td></tr>';
}
...
# Bugnote Text Box
echo '<tr>';
echo '<th class="category"><label for="bugnote_text">' . _( 'Add Note' ) . '</label></th>';
