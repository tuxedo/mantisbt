...
$c_filter['assigned'] = array(
	...
	FILTER_PROPERTY_HIDE_STATUS => array( '0' => $t_bug_resolved_status_threshold ),
);
...
$c_filter['recent_mod'] = array(
	FILTER_PROPERTY_HIGHLIGHT_CHANGED => $t_default_show_changed,
	FILTER_PROPERTY_HIDE_STATUS => array( '0' => META_FILTER_NONE ),
);
...
if( is_array( $t_rows ) && helper_get_current_project() == 0 ) {
	$t_categories = array();
...
<tr class="my-buglist-bug <?php echo $t_bug_class?> <?php echo $status_label ?>">
	...
	if( !bug_is_readonly( $t_bug ) && access_has_bug_level( config_get( 'update_bug_threshold' ), $t_bug ) ) {
